...
const PostDetail = require("../models/postDetailsModel");
const { ResultMessage } = require("../pattern/response/resultMessage");
const { POST_TYPE, MESSAGE, CODE } = require("../constants");
...
const createPostSubCategory = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (postDetailCheck.user_id.toString() !== req.user.id) {
...
const deletePostCategoryDetail = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (postDetail.user_id.toString() !== req.user.id) {
            return res.status(200).json(new ResultMessage(CODE.CREDENTIAL, MESSAGE.CREDENTIAL));
        }
    } catch (error) {
        return res.status(200).json(new ResultMessage(CODE.NOT_FOUND, MESSAGE.NOT_FOUND));
